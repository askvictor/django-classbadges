...
def award_badge_view(request):
    if request.method=="POST" and request.is_ajax():
        ...
        if request.POST['action'] == 'award':
            ...
            if a:
                response = simplejson.dumps({"awarded": "yes", "student":s.username, "badge":b.slug})
            else:
                response = simplejson.dumps({"awarded": "no"})
            return HttpResponse(response, content_type="application/json")
        elif request.POST['action'] == 'unaward':
            a = get_object_or_404(Award, badge=b, user=s)
            a.delete()
            response = simplejson.dumps({"unawarded": "yes", "student":s.username, "badge":b.slug})
            return HttpResponse(response, content_type="application/json")
